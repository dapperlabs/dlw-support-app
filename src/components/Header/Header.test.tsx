...
test('renders header with sign-in button when not logged in', () => {
    const { getByText } = render(
        <BrowserRouter>
            <Header loggedIn={''} handleSignIn={handleSignIn} handleSignOut={handleSignOut} isDapper={false} BASE_URL={BASE_URL} />
        </BrowserRouter>
    )
...
test('renders header with sign-out button when logged in', () => {
    const { getByText } = render(
        <BrowserRouter>
            <Header loggedIn={'0x456'} handleSignIn={handleSignIn} handleSignOut={handleSignOut} isDapper={false} BASE_URL={BASE_URL} />
        </BrowserRouter>
    )
...
test('displays Dapper logo when isDapper is true', () => {
    const { getByAltText } = render(
        <BrowserRouter>
            <Header loggedIn={'0x456'} handleSignIn={handleSignIn} handleSignOut={handleSignOut} isDapper={true} BASE_URL={BASE_URL} />
        </BrowserRouter>
    )
